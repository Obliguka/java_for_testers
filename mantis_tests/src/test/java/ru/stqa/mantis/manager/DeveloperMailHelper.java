...
public class DeveloperMailHelper extends HelperBase{
    ...
    public String recieve(DeveloperMailUser user, Duration duration) {
        ...
        while (System.currentTimeMillis() < start + duration.toMillis()) {
            try {
                ...
                if(response1.result().size()>0){
                    var text2=get(String.format(
                                    "https://www.developermail.com/api/v1/mailbox/%s/messages/%s",user.name(),response1.result().get(0))
                            , user.token());
